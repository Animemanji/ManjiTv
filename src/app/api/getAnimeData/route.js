...
export const GET = async () => {
  try {
    // Fetch subfolders under 'anime' folder
    const folders = await cloudinary.v2.api.sub_folders('anime');
    ...
    const animeDataPromises = folders.folders.map(async (folder) => {
      try {
        ...
        const coverImageRes = await cloudinary.v2.api.resources({
          type: 'upload',
          prefix: `anime/${animeTitle}/cover`,
          resource_type: 'image',
          max_results: 1,
          context: true // Ensure context is fetched
        });
        
        // Check if cover image and description are returned
        ...
        console.log('Cover Image Response:', coverImageRes.resources[0]); // Debug log

        // Fetch all video episodes and their descriptions
        const resources = await cloudinary.v2.api.resources({
          type: 'upload',
          prefix: `anime/${animeTitle}`,
          resource_type: 'video',
          context: true // Ensure context is fetched
        });
